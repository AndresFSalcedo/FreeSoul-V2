...
function ProductList(props) {
  ...
  let uniqueImagesGalery = props.products.filter(
    (prodFilter, index) =>
      index ===
      props.products.findIndex(
        (prodRemove) => prodRemove.productCode === prodFilter.productCode
      )
  );
  ...
  return (
    ...
    <div className="product-list">
      {uniqueImagesGalery.map((product) => {
        return <Product key={product._id} product={product} allImages={props.products}/>;
      })}
    </div>
  );
}
